...
class ContentTypeBuilderTest extends WebTestBase {
     ...
     * Modules to enable.
     */
    public static $modules = array('rdf_builder','rdfui', 'rdf', 'field','node','entity');
    ...
    function testContentTypeCreate(){
        $this->edit_form_one();

        foreach(array('email','name') as $element){
            $this->assertText($element, format_string('property "@element" of "@type" was found.', array('@element' => $element, '@type'=>$this->rdf_type)));
        }
