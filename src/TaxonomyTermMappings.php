...
class TaxonomyTermMappings {
  ...
  public static function alterForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!$entity_type->isNew()) {
      $existing_type = rdf_get_mapping('taxonomy_term', $entity_type->id())->getBundleMapping();
    }
  ...
  public static function submitForm(array &$form, FormStateInterface $form_state) {
    if ($form_state->hasValue('types')) {
      /** @var \Drupal\Core\Entity\EntityInterface $entity_type */
      $entity_type = $form_state->getFormObject()->getEntity();
      $mapping = rdf_get_mapping('taxonomy_term', $entity_type->id());
      if ($entity_type->isNew()) {
        $mapping = rdf_get_mapping('taxonomy_term', $form_state->getValue('types'));
      }
